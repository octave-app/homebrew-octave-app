class Python370 < Formula
  ...
  depends_on "sphinx-doc_1.7.6" => :build
  depends_on "gdbm_1.17"
  depends_on "openssl_1.0.2p"
  ...
  def install
    ...
    args = %W[
      ...
      --without-ensurepip
      --with-dtrace
      --with-openssl=#{Formula["openssl_1.0.2p"].opt_prefix}
    ]
  ...
  def post_install
    ...
    include_dirs = [HOMEBREW_PREFIX/"include", Formula["openssl_1.0.2p"].opt_include,
                    Formula["sqlite_3.24.0"].opt_include]
    library_dirs = [HOMEBREW_PREFIX/"lib", Formula["openssl_1.0.2p"].opt_lib,
