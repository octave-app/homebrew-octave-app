class Libtiff4095 < Formula
  ...
  depends_on "xz_5.2.4_0" => :optional
  ...
  def install
    ...
    if build.with? "xz"
      args << "--with-lzma-include-dir=#{Formula["xz_5.2.4_0"].opt_include}"
      args << "--with-lzma-lib-dir=#{Formula["xz_5.2.4_0"].opt_lib}"
    else
      args << "--disable-lzma"
    end
