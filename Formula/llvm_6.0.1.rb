...
class Llvm601 < Formula
  ...
  depends_on "libffi_3.2.1" => :recommended

  # for the 'dot' tool (lldb)
  depends_on "graphviz_2.40.1" => :optional

  depends_on "ocaml_4.07.0" => :optional
  if build.with? "ocaml"
  ...
  def install
    ...
    if build.with? "python@2"
      ENV.prepend_path "PATH", Formula["python_2.7.15"].opt_libexec/"bin"
    end
    ...
    if build.with? "libffi"
      ...
      args << "-DFFI_INCLUDE_DIR=#{Formula["libffi_3.2.1"].opt_lib}/libffi-#{Formula["libffi_3.2.1"].version}/include"
      args << "-DFFI_LIBRARY_DIR=#{Formula["libffi_3.2.1"].opt_lib}"
    end
