class Python27151 < Formula
  ...
  depends_on "sphinx-doc_1.8.1_0" => :build if MacOS.version > :snow_leopard
  depends_on "gdbm_1.18.1_0"
  depends_on "openssl_1.0.2p_0"
  depends_on "readline_7.0.5_0"
  depends_on "sqlite_3.25.3_0"
  depends_on "tcl-tk_8.6.8_0" => :optional
  ...
  def install
    ...
    inreplace "setup.py" do |s|
      s.gsub! "do_readline = self.compiler.find_library_file(lib_dirs, 'readline')",
              "do_readline = '#{Formula["readline_7.0.5_0"].opt_lib}/libhistory.dylib'"
      s.gsub! "/usr/local/ssl", Formula["openssl_1.0.2p_0"].opt_prefix
    end

    inreplace "setup.py" do |s|
      s.gsub! "sqlite_setup_debug = False", "sqlite_setup_debug = True"
      s.gsub! "for d_ in inc_dirs + sqlite_inc_paths:",
              "for d_ in ['#{Formula["sqlite_3.25.3_0"].opt_include}']:"
    ...
    if build.with? "tcl-tk"
      tcl_tk = Formula["tcl-tk_8.6.8_0"].opt_prefix
  ...
  def post_install
    ...
    include_dirs = [HOMEBREW_PREFIX/"include", Formula["openssl_1.0.2p_0"].opt_include,
                    Formula["sqlite_3.25.3_0"].opt_include]
    library_dirs = [HOMEBREW_PREFIX/"lib", Formula["openssl_1.0.2p_0"].opt_lib,
                    Formula["sqlite_3.25.3_0"].opt_lib]

    if build.with? "tcl-tk"
      include_dirs << Formula["tcl-tk_8.6.8_0"].opt_include
      library_dirs << Formula["tcl-tk_8.6.8_0"].opt_lib
    end
