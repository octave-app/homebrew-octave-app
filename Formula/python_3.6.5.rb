class Python365 < Formula
  ...
  depends_on "sphinx-doc_1.7.5" => :build
  depends_on "gdbm_1.15"
  depends_on "openssl_1.0.2o"
  depends_on "readline_7.0.3"
  depends_on "sqlite_3.24.0"
  depends_on "xz_5.2.4"
  depends_on "tcl-tk_8.6.8" => :optional
  ...
  def install
    ...
    inreplace "setup.py",
      ...
      "do_readline = '#{Formula["readline_7.0.3"].opt_lib}/libhistory.dylib'"

    if build.stable?
      inreplace "setup.py", "/usr/local/ssl", Formula["openssl_1.0.2o"].opt_prefix
    else
      args << "--with-openssl=#{Formula["openssl_1.0.2o"].opt_prefix}"
    end

    inreplace "setup.py" do |s|
      s.gsub! "sqlite_setup_debug = False", "sqlite_setup_debug = True"
      s.gsub! "for d_ in inc_dirs + sqlite_inc_paths:",
              "for d_ in ['#{Formula["sqlite_3.24.0"].opt_include}']:"
    end
    ...
    if build.with? "tcl-tk"
      tcl_tk = Formula["tcl-tk_8.6.8"].opt_prefix
  ...
  def post_install
    ...
    include_dirs = [HOMEBREW_PREFIX/"include", Formula["openssl_1.0.2o"].opt_include,
                    Formula["sqlite_3.24.0"].opt_include]
    library_dirs = [HOMEBREW_PREFIX/"lib", Formula["openssl_1.0.2o"].opt_lib,
                    Formula["sqlite_3.24.0"].opt_lib]

    if build.with? "tcl-tk"
      include_dirs << Formula["tcl-tk_8.6.8"].opt_include
      library_dirs << Formula["tcl-tk_8.6.8"].opt_lib
    end
