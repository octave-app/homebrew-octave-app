class Rust1280 < Formula
  ...
  def install
    ...
    ENV["OPENSSL_INCLUDE_DIR"] = Formula["openssl_1.0.2p"].opt_include
    ENV["OPENSSL_LIB_DIR"] = Formula["openssl_1.0.2p"].opt_lib
    ...
    args << "--llvm-root=#{Formula["llvm_6.0.1"].opt_prefix}" if build.with? "llvm"
    if build.head?
